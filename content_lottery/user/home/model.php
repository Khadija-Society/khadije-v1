...
class model
{
	...
	public static function export_list()
	{
		...
		$link                = null;
		$result              = \lib\db\karbala2users::search(null, $_args);

		while ($result)
		{
			$result = array_map(["\\lib\\app\\karbala2user", "ready"], $result);
	...
	{
		$handle   = curl_init();
		curl_setopt($handle, CURLOPT_URL, \dash\url::kingdom().'/a/exportcurlkarbala2users');
