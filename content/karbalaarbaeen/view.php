<?php
namespace content\karbalaarbaeen;
...
class view
{
	...
	{
		\dash\data::page_title("ثبت‌نام کربلا ویژه اربعین");
		\dash\data::page_desc(\dash\data::site_desc());

		\dash\data::userdetail(\dash\db\users::get(['id' => \dash\user::id(), 'limit' => 1]));

		if(isset($_SESSION['NEW_KARBALA_SIGNUP_karbalaarbaeen']))
		{
			$_SESSION['NEW_KARBALA_SIGNUP_karbalaarbaeen'] = null;
			\dash\data::newKarbalaSignup(true);
		}

		$countSingupKarbala = \lib\db\karbalaarbaeenusers::get_last_id();
	...
	private static function check_nationalcode()
	{
		...
		$check = \lib\db\karbalaarbaeenusers::get(['nationalcode' => $nationalcode, 'limit' => 1]);
		if(isset($check['id']))
		{
			$status = 'signuped';
		}
