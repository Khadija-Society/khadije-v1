<?php
class TransStatic
{
 private function trans_static()
 {
	echo T_("Khadije");

	echo T_(" ");

	echo T_("money");
	echo T_("toman");

	echo T_("Enable");
	echo T_("Disable");
	echo T_("Expire");
	echo T_("Deleted");
	echo T_("Lock");
	echo T_("Awaiting");
	echo T_("Cancel");
	echo T_("Accept");
	echo T_("Reject");
	echo T_("Spam");
	echo T_("enable");
	echo T_("disable");
	echo T_("expire");
	echo T_("deleted");
	echo T_("lock");
	echo T_("awaiting");
	echo T_("cancel");
	echo T_("accept");
	echo T_("reject");
	echo T_("spam");
	echo T_("cpSettingThankyouMessage");

	echo T_("mashhad");
	echo T_("karbala");
	echo T_("qom");
	echo T_("najaf");
	echo T_("Najaf");
	echo T_("Mecca");
	echo T_("mecca");
	echo T_("syria");
	echo T_("Syria");
	echo T_("Group");


	echo T_("ControlPanel");
	echo T_("cpTripEdit");
	echo T_("cpTripView");
	echo T_("cpTripOption");
	echo T_("donateManualPay");
	echo T_("cpDonateView");
	echo T_("cpDonateTotalPay");
	echo T_("cpDonateOption");
	echo T_("cpNationalCodeEdit");
	echo T_("cpNationalCodeView");
	echo T_("cpNationalCodeImport");
	echo T_("cpHomePageNumber");
	echo T_("cpOptionProduct");
	echo T_("cpOptionCityPlace");
	echo T_("cpServiceView");
	echo T_("cpServiceOption");
	echo T_("cpTripQom");
	echo T_("cpTripMashhad");
	echo T_("cpTripKarbala");
	echo T_("Control");
	echo T_("Trip");
	echo T_("Donate");
	echo T_("National");
	echo T_("National Code");
	echo T_("Setting");
	echo T_("Service");

	echo T_("cpBookTransactionView");
	echo T_("mylovestory");

	echo T_("cpReportDaily");
	echo T_("cpReportMonth");
	echo T_("cpReportView");

	echo T_("Delneveshte");
	echo T_("Admin Contact");
	echo T_("Representation");
	echo T_("Consulting");
	echo T_("Health");
	echo T_("cpDelneveshteEdit");
	echo T_("cpDelneveshteView");
	echo T_("cpAdminContactChangeStatus");
	echo T_("cpAdminContactView");
	echo T_("cpRepresentationChangeStatus");
	echo T_("cpRepresentationView");
	echo T_("cpRepresentationOption");
	echo T_("cpConsultingView");
	echo T_("cpConsultingOption");
	echo T_("cpHealthView");
	echo T_("cpHealthOption");

	echo T_("Festival");
	echo T_("cpFestivalEdit");
	echo T_("cpFestivalAdd");
	echo T_("cpFestivalView");
	echo T_("cpFestivalUsers");

	echo T_("cpFestivalUsersSignupEdit");

	echo T_("cpFestivalMediasupporterAdd");
	echo T_("cpFestivalMediasupporterEdit");
	echo T_("cpFestivalMediasupporterView");

	echo T_("cpFestivalRefereeAdd");
	echo T_("cpFestivalRefereeEdit");
	echo T_("cpFestivalRefereeView");

	echo T_("cpFestivalOrganizerAdd");
	echo T_("cpFestivalOrganizerEdit");
	echo T_("cpFestivalOrganizerView");

	echo T_("cpFestivalSupporterAdd");
	echo T_("cpFestivalSupporterEdit");
	echo T_("cpFestivalSupporterView");

	echo T_("cpFestivalCourseAdd");
	echo T_("cpFestivalCourseEdit");
	echo T_("cpFestivalCourseView");

	echo T_("cpFesitvalTransaction");

	// ----------------------------- users table
	echo T_("married");
	echo T_("zipcode");
	echo T_("desc");
	echo T_("job");
	echo T_("iscompleteprofile");
	echo T_("nesbat");

	echo T_("cpChartCountTransaction");
	echo T_("Report");
	echo T_("cpDisableAllTrip");
	echo T_("Danger");
	echo T_("cpHadith");
	echo T_("Hadith");

	echo T_("cpFamilyTripView");
	echo T_("cpFamilyTripEdit");
	echo T_("Family Trip");
	echo T_("cpFamilyTripOption");

	echo T_("cpMeeting");
	echo T_("cpMeetingManage");
	echo T_("sort");
	echo T_("Sort");

	echo T_("cpConsultingChangeStatus");
	echo T_("cpServiceChangeStatus");
	echo T_("cpHealthChangeStatus");
	echo T_("Student");
	echo T_("student");
	echo T_("collegian");
	echo T_("Collegian");
 }
}
?>