...
class lotterywin
{
	...
	public static function load_lottery($_table, $_md5, $_step)
	{
		...
		$load_user = \lib\db\lottery_user::get(['id' => [" IN ", " ($ids)"]]);
	...
	public static function remove($_id)
	{
		...
		{
			\lib\db\lottery_user::remove_lottery_id($_id);
	...
	public static function add($_args)
	{
		...
		if($id)
		{
			$win = \lib\db\lottery_user::get_rand($args['countall'], $args['table']);
			...
			if($win && is_array($win))
			{
				...
				\lib\db\lottery_user::update_win(implode(',', $win), $id);
			}
		}
