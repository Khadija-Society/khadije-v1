...
class karbala2user
{
	...
	{
		$list = \lib\db\karbala2users::chart_province();
		$result = [];
		if(is_array($list))
		{
	...
	{
		$result     = \lib\db\karbala2users::daily_chart();
		$hi_chart   = [];
		$categories = [];
		$values     = [];
	...
	public static function add($_args, $_option = [])
	{
		...
		if(isset($args['nationalcode']))
		{
			...
			$check_duplicate = \lib\db\karbala2users::get($check_duplicate);
		...
		$id = \lib\db\karbala2users::insert($args);
