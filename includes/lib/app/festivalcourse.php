...
class festivalcourse
{
	...
	private static function check($_id = null)
	{
		...
		{
			\dash\notif::error(T_("Please fill the festivalcourse title"), 'title');
			return false;
		}
		...
		{
			\dash\notif::error(T_("Please fill the festivalcourse title less than 500 character"), 'title');
			return false;
		}

		if($title)
		{
			$check_duplicate = \lib\db\festivalcourses::get(['title' => $title, 'limit' => 1]);
		...
		{
			\dash\notif::error(T_("Please fill the festivalcourse subtitle less than 500 character"), 'subtitle');
			return false;
		}
		...
		{
			\dash\notif::error(T_("Please fill the festivalcourse slug less than 500 character"), 'slug');
			return false;
		}
		...
		if(\dash\app::isset_request('slug'))
		{
			if(!$slug)
			{
				\dash\notif::error(T_("slug of festivalcourse can not be null"), 'slug');
				return false;
			}

			if(mb_strlen($slug) < 3)
			{
				\dash\notif::error(T_("festivalcourse slug must be larger than 3 character"), 'slug');
				return false;
			}
			...
			{
				\dash\notif::error(T_("Only [A-Za-z0-9] can use in festivalcourse slug"), 'slug');
				return false;
			}
			...
			{
				\dash\notif::error(T_("festivalcourse slug must be less than 500 character"), 'slug');
				return false;
			}

			$slug = mb_strtolower($slug);

			$check_duplicate = \lib\db\festivalcourses::get(['slug' => $slug, 'language' => \dash\language::current(), 'limit' => 1]);
			...
			if(isset($check_duplicate['id']))
			{
				...
				{
					\dash\notif::error(T_("Duplicate festivalcourse slug"), 'slug');
					return false;
				}
			}
		...
		{
			\dash\notif::error(T_("Please fill the festivalcourse slug"), 'slug');
			return false;
		}
